...
window.onload = function() {
    document.getElementById("newFormButton").click();
};
...
function createNewForm() {
    ...
    formContainer.innerHTML = `
        <form class="calculatorForm" id="priceCalculatorForm${formIndex}">
    ...
    ['woodType', 'thickness', 'length', 'width', 'piece', 'varnish', 'stain', 'oil', 'mill'].forEach(function(id) {
        document.getElementById(`${id}${formIndex}`).addEventListener('input', function() {
...
function calculatePrice(formIndex) {
    ...
    var priceForWider = width > 65 ? 1.2 : 1;
    ...
    if (mill) additionalPrice += 10.00; // 10zł za frezowanie
...
function updateCheckboxState(formIndex) {
    ['varnish', 'stain', 'oil', 'mill'].forEach(function(id) {
        document.getElementById(id + formIndex).addEventListener('change', function() {
            this.value = this.checked ? 'Tak' : 'Nie';
        });
    });
}

function updateOverallPrices() {
    const elementsWithPrice1 = document.querySelectorAll('[id^="result1"]');
    let totalPriceAl1 = 0;
    elementsWithPrice1.forEach(element => {
        let price = parseFloat(element.innerText.split(" ")[2]);
        if (!isNaN(price)) {
            totalPriceAl1 += price;
        }
    });
    
    const elementsWithPrice2 = document.querySelectorAll('[id^="result2"]');
    let totalPriceAl2 = 0;
    elementsWithPrice2.forEach(element => {
        let price = parseFloat(element.innerText.split(" ")[3]);
        if (!isNaN(price)) {
            totalPriceAl2 += price;
        }
    });

    const elementsWithPrice3 = document.querySelectorAll('[id^="result3"]');
    let totalPriceAl3 = 0;
    ...
    document.getElementById('overallPrice1').innerText = 'Całkowita cena Allegro: ' + totalPriceAl1.toFixed(2) + ' zł.';
    document.getElementById('overallPrice2').innerText = 'Całkowita cena poza Allegro: ' + totalPriceAl2.toFixed(2) + ' zł.';
    document.getElementById('overallPrice3').innerText = 'Do kupienia przez Allegro: ' + Math.ceil(totalPriceAl3) + ' szt.';
}
